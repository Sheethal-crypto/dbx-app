...
h1 { font-size: 40px; font-weight: 700; letter-spacing: -0.024em; color: var(--ink);
...
.vsf-brand { font-size: 26px; font-weight: 700; letter-spacing: -0.02em; color: var(--ink); }
.vsf-chip {
...
.vsf-component { font-size: 18px; font-weight: 650; color: var(--ink); }
...
def fill_synthesis(slot, results, vehicle):
    """Run the agent and write the card into its reserved slot.

    The call is made outside the slot so the skeleton stays visible while it
    runs, and any failure is rendered into the slot rather than at the foot of
    ...
    slot.markdown(synthesis_card(body), unsafe_allow_html=True)
...
def render_ask():
    ...
    rule()
    # The summary sits above the evidence but is the slowest call on the page,
    # so only its slot is claimed here. The complaints below render immediately
    # and the slot is filled once they are on screen.
    scoped = st.session_state.results_scoped
    synthesis_slot = reserve_synthesis(scoped and vehicle is not None)
    ...
    # Last on the page, written back into the slot at the top. Everything the
    # user can read or click is already painted before this blocks.
    if synthesis_slot is not None:
        fill_synthesis(synthesis_slot, results, vehicle)

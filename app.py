...
# Optional. Databricks Apps serve Streamlit inside an iframe, and microphone
# access can be refused by the platform's Permissions-Policy, so nothing here
# may be load bearing. Absent or broken, the textarea still works.
try:
    from streamlit_mic_recorder import speech_to_text
except Exception:  # noqa: BLE001 - voice is an enhancement, never a dependency
    speech_to_text = None
...
/* hero, empty garage only. The image is a data URI, never a remote fetch. */
.vsf-hero {
  position: relative; height: 260px; border-radius: 14px; overflow: hidden;
  display: flex; flex-direction: column; justify-content: center;
  padding: 0 36px 96px; margin-bottom: 8px;
}
.vsf-hero img {
...
.vsf-hero::after {
  ...
  background: linear-gradient(180deg, rgba(4, 47, 43, 0.42) 0%, rgba(6, 61, 56, 0.68) 100%);
  opacity: 0.55;
}
.vsf-hero-inner { position: relative; z-index: 2; }
.vsf-hero-title {
  font-size: 30px; font-weight: 700; letter-spacing: -0.02em; color: #FFFFFF;
  line-height: 1.2; margin-bottom: 8px; text-shadow: 0 1px 12px rgba(0, 0, 0, 0.35);
}
.vsf-hero-body {
  font-size: 17px; line-height: 1.55; color: rgba(255, 255, 255, 0.92); max-width: 46ch;
  text-shadow: 0 1px 10px rgba(0, 0, 0, 0.35);
}
/* Lift the next Streamlit block, the VIN form, onto the lower band of the hero.
   Both testid spellings are covered. If neither matches the form simply sits
   below the image, which still reads correctly. */
[data-testid="stElementContainer"]:has(.vsf-hero-hook) + [data-testid="stElementContainer"],
.element-container:has(.vsf-hero-hook) + .element-container {
  margin-top: -88px; position: relative; z-index: 3;
}

/* mic button. The component itself renders in an iframe, so only its frame is
   reachable from here; the glyph beside it is ours. */
.vsf-mic-label {
  display: flex; align-items: center; gap: 8px; height: 44px;
  font-size: 15px; color: var(--ink-soft);
}
iframe[title^="streamlit_mic_recorder"] {
  height: 46px !important; border-radius: 10px; background: transparent;
}
...
.stButton > button[kind="primary"]:active { transform: translateY(1px); box-shadow: none; }
...
MIC_SVG = (
    ...
    "<rect x='9' y='3' width='6' height='11' rx='3' stroke='#0F766E' stroke-width='2'/>"
    "<path d='M5 11a7 7 0 0014 0M12 18v3' stroke='#0F766E' stroke-width='2'"
    " stroke-linecap='round'/></svg>"
)

HERO_PATH = Path(__file__).resolve().parent / "static" / "hero.jpg"


# ------------------------------------------------------------------- helpers

@st.cache_data(show_spinner=False)
def _encode_hero(path_str, mtime, size):
    """Inline the hero as a data URI.

    Streamlit does not serve arbitrary static files reliably, and a remote URL
    would hang rather than fail, since outbound access is limited to a trusted
    domain list. Keyed on mtime and size so replacing the file busts the cache.
    """
    del mtime, size  # key only
...
def hero_uri():
    """The encoded hero, or None when the file is absent or unreadable."""
    try:
        stat = HERO_PATH.stat()
        return _encode_hero(str(HERO_PATH), stat.st_mtime, stat.st_size)
    except Exception:  # noqa: BLE001 - the SVG empty state covers every failure
        return None
...
def render_hero(uri):
    """Banner for the empty garage. The trailing hook is what the stylesheet
    uses to lift the VIN form up onto the lower band of the image."""
    st.markdown(
        "<div class='vsf-hero'><img src='{uri}' alt=''>"
        "<div class='vsf-hero-inner'>"
        "<div class='vsf-hero-title'>Add your first vehicle</div>"
        "<div class='vsf-hero-body'>Enter a VIN. It is decoded through NHTSA vPIC, "
        "and every search after that is scoped to your car.</div>"
        "</div></div><div class='vsf-hero-hook'></div>".format(uri=uri),
        unsafe_allow_html=True,
    )
...
def render_garage():
    load_vehicles()
    hero = hero_uri() if not (st.session_state.vehicles or []) else None
...
def render_mic():
    """Optional voice entry for the symptom box.

    Everything here is best effort. The component is a custom Streamlit iframe,
    and Databricks Apps may refuse it microphone access through the platform's
    Permissions-Policy, so an import failure, a runtime error or an empty
    transcript all leave typing as the only input and say nothing about it.
    """
    if speech_to_text is None:
        return
    st.markdown(
        "<div class='vsf-mic-label'>{}Voice</div>".format(MIC_SVG),
        unsafe_allow_html=True,
    )
    try:
    ...
    except Exception:  # noqa: BLE001 - a blocked microphone must not break the page
        return
    if transcript:
        # Fill the box and stop. The user reads and edits before searching.
        st.session_state.symptom = transcript
        st.rerun()
...
def synthesis_prompt(symptom, vehicle, results):
    ...
    return (
        ...
        "These are the complaints the app returned, all for this vehicle:\n"
        "{listed}\n\n"
...
def render_synthesis(results, vehicle):
    """Reads the results back to the owner. Runs itself once results exist."""
    if not st.session_state.results_scoped or vehicle is None:
        st.markdown(
            "<div class='vsf-synthesis is-muted'>"
            "<div class='vsf-synthesis-title'>What this means</div>"
    ...
    if sections:
        body = "".join(
            ...
            "<div class='vsf-syn-body{}'>{}</div></div>".format(
                esc(label), " mono" if label == "Sources" else "", esc(sections[label])
            )
    ...
    st.markdown(
        "<div class='vsf-synthesis'>"
        "<div class='vsf-synthesis-title'>What this means</div>{}</div>".format(body),
        unsafe_allow_html=True,
    )
...
def render_year_chart(results, vehicle):
    ...
    st.caption("From the {} results above, not all complaints.".format(len(results)))
...
def render_ask():
    ...
    text_column, mic_column = st.columns([9, 1.6])
    with text_column:
    ...
    with mic_column:
        # May rerun with a transcript, which is why it comes after the textarea.
        render_mic()
    ...
    for position, match in enumerate(results):
        render_complaint_card(
            match, vehicle, st.session_state.results_scoped, delay_ms=position * 60
        )

    render_synthesis(results, vehicle)

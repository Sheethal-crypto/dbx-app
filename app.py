...
def render_insights():
    ...
    lede(
        ...
        "a Delta table with change data feed enabled. These figures come from "
        "workspace.vsf.app_events_analytics, which this tab rebuilds from that change "
        "feed, so the app reports on itself."
    )

    freshness, refresh = st.columns([3, 1])
    with freshness:
        st.caption("Rebuilt from the change feed, cached for up to 60 seconds.")
    with refresh:
    ...
    rule()
    eyebrow("Tool usage")
    with st.spinner("Rebuilding from the change feed"):

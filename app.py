...
Navigation is st.tabs. Streamlit renders every tab body on every rerun, so the
two Insights queries would fire on every interaction, not only when that tab is
open. They are wrapped in a 60 second cache for that reason, with a Refresh
button on the tab to clear it.
...
html, body, [class*="css"] { font-family: var(--sans); font-size: 16px; color: var(--ink); }
...
h1 { font-size: 30px; font-weight: 700; letter-spacing: -0.02em; color: var(--ink);
     margin: 0 0 8px; line-height: 1.25; }
h2 { font-size: 20px; font-weight: 650; color: var(--ink); }
h3 { font-size: 17px; font-weight: 650; color: var(--ink); }
p, li, label, .stMarkdown { font-size: 16px; }

/* header */
.vsf-header {
  display: flex; align-items: center; justify-content: space-between;
  gap: 16px; flex-wrap: wrap; padding: 4px 0 20px;
}
.vsf-brand { font-size: 20px; font-weight: 700; letter-spacing: -0.015em; color: var(--ink); }
.vsf-chip {
  display: inline-flex; align-items: center; gap: 8px;
  background: var(--accent); color: #FFFFFF; border-radius: 999px;
  padding: 9px 18px; font-size: 14px; font-weight: 600; letter-spacing: 0.01em;
  box-shadow: var(--lift);
}
...
.stTabs [data-baseweb="tab-list"] {
  gap: 28px; background: transparent; border-bottom: 1px solid var(--hairline);
  margin-bottom: 28px;
}
.stTabs [data-baseweb="tab"] {
  height: 48px; padding: 0; background: transparent;
  font-size: 16px; font-weight: 550; color: var(--ink-soft);
}
...
.vsf-lede { font-size: 16px; line-height: 1.65; color: var(--ink-soft); margin: 0 0 28px; }
.vsf-rule { height: 32px; }
...
.vsf-vehicle-title { font-size: 17px; font-weight: 650; color: var(--ink); letter-spacing: -0.01em; }
.vsf-vehicle-meta { font-size: 14px; color: var(--ink-soft); margin-top: 4px; }
.vsf-vin {
...
.vsf-component { font-size: 15px; font-weight: 650; color: var(--ink); }
.vsf-year { font-size: 14px; color: var(--ink-soft); white-space: nowrap; }
.vsf-narrative {
  font-size: 15px; line-height: 1.7; letter-spacing: 0.015em;
  color: var(--ink-soft); margin: 14px 0 16px;
}
.vsf-complaint-id { font-family: var(--mono); font-size: 13px; color: var(--ink-soft); }
.vsf-badge {
  display: inline-block; font-size: 13px; font-weight: 600; color: var(--amber);
...
.stTextInput input, .stTextArea textarea {
  background: var(--card); border: none; font-size: 16px; color: var(--ink);
}
...
[data-testid="stWidgetLabel"] p, [data-testid="stCaptionContainer"] p { font-size: 14px; }

/* tables */
.vsf-table { width: 100%; border-collapse: collapse; font-size: 15px; }
.vsf-table th {
...
.vsf-table td.mono { font-family: var(--mono); font-size: 14px; color: var(--ink-soft); }
...
[data-testid="stVerticalBlockBorderWrapper"] > div { padding: 8px; }

.vsf-empty {
  background: var(--card); border-radius: 14px; box-shadow: var(--lift);
  padding: 24px; font-size: 16px; color: var(--ink-soft);
}
.vsf-footer {

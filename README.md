...
Those calls retry connection errors, timeouts, 429 and 5xx with exponential backoff over
three attempts. Other 4xx are deliberately not retried: the 400 above is an answer rather
than a fault, and retrying it would turn one correct fast skip into three slow ones. When
a call does exhaust its attempts, the result handed back to the model is marked
unavailable, so it reports a failed lookup rather than an absence of records.
...
Insights tab reads the analytics table rebuilt from its change feed. The app reports on
itself.
...
`tool_usage()` in `app/vsf_tools.py` runs that statement and then reads the table, so
the Insights tab is driven by the change feed rather than by a second aggregation over
the event log. A failed rebuild is swallowed and the tab falls back to whatever the
table already holds.
...
The deployed app is at https://dbx-app-7474654713254531.aws.databricksapps.com. Access
requires a Databricks workspace login, and Free Edition stops the app 24 hours after it
starts, so it may need restarting. A reviewer without workspace access should refer to
`screenshots/`.

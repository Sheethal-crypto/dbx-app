...
|---|---|
| Data pipeline in Spark | `notebooks/01_ingest_bronze`, `02_silver_transform`, `03_gold_and_index`. 2,230,776 raw complaints down to 35,269 curated chunks |
...
| Processing unstructured data | 2.2 million free-text complaint narratives, embedded with `databricks-gte-large-en` into a Delta Sync vector index |
...
Across 63 logged tool calls, `search_complaints` runs at 611ms p50 and `decode_vin` at
314ms.

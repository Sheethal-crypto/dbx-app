...
def search_complaints(symptom, model_family=None, num_results=5):
    ...
    one vehicle line."""
    kwargs = dict(query_text=symptom, columns=SEARCH_COLUMNS, num_results=num_results * 2)
    if model_family:
...
def add_vehicle(vin):
    ...
    r = requests.get(VPIC_DECODE.format(vin=vin), timeout=20)
    r.raise_for_status()
    d = r.json()["Results"][0]
    ...
    family = to_model_family(model_raw)
    ...
    run_sql(
        ...
        {
            "vehicle_id": vehicle_id, "user_id": USER_ID, "vin": vin, "make": make,
            "model_raw": model_raw, "model_family": family, "model_year": year,
        },
    )

    return {"vehicle_id": vehicle_id, "vin": vin, "make": make,
            "model_raw": model_raw, "model_family": family, "model_year": year}
...
def chat_client():
    ...
    user in a notebook, the service principal in the app. Requires the SDK's openai
    extra."""
    global _chat
    if _chat is None:
        _chat = _w.serving_endpoints.get_open_ai_client()
    return _chat

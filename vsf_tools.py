...
def tool_usage():
    """Rebuild the CDF-derived analytics table, then read the Insights figures from it.

    The rebuild is the point. The table is materialized from
    `table_changes('...app_events', 0)`, so the tab reads the change feed rather than
    aggregating the base table a second time. The statement is the one documented in
    `notebooks/06_cdf_analytics.md`, kept identical so the code, the doc and screenshot
    20 cannot drift apart.

    A failed rebuild is swallowed rather than raised. The read below then returns
    whatever the table already holds, which is stale rather than absent, and a working
    tab is worth more than a current number.

    Two costs worth knowing. The rebuild rescans the change feed from version 0 on every
    Insights cache miss, so its cost grows without bound as app_events fills; a watermark
    or an incremental rebuild is the fix if it starts to hurt. And `event_type =
    'tool_call'` is stricter than the `tool_name IS NOT NULL` it replaced, so a future
    event carrying a tool_name without being a tool call, a retry or a timeout say, would
    drop out of these figures silently.
    """
    try:
        run_sql(
            f"""CREATE OR REPLACE TABLE {FQ}.app_events_analytics AS
                SELECT tool_name,
                       count(*) AS calls,
                ...
                FROM table_changes('{FQ}.app_events', 0)
                WHERE _change_type = 'insert' AND event_type = 'tool_call'
                GROUP BY tool_name
                ORDER BY calls DESC"""
        )
    except Exception as exc:  # noqa: BLE001 - a stale table beats a broken tab
        log_event("analytics_rebuild", None, {"error": str(exc)[:500]}, None, False)

    # Five columns in this order: app.py zips rows into dicts by position, and the
    # table's sixth column, last_change, is not one of them. round() restores the
    # integer display the previous percentile_approx query produced.
    return run_sql(
        f"""SELECT tool_name,
                   calls,
                   round(p50_ms) AS p50_ms,
                   round(p95_ms) AS p95_ms,
                   failures
            FROM {FQ}.app_events_analytics
            ORDER BY calls DESC"""
    )
